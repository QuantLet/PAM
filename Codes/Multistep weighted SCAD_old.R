...
XV.fct = function(X, beta, lambda, a){
  ...
  if (max(pen) == 0){
    ...
    for (j in 1:length(V.index)){
      X.V[, j] = X.V[, j] * lambda / (pen.new[j] * wght.new[j]) 
    }
  }
  X.V
}
...
V.coeff.fct = function(coeff, beta, lambda, a){
  ...
  V.coeff   = numeric(0)
  for (j in 1:length(V.index)){
    V.coeff[j] = coeff[j] * lambda / (pen.new[j] * wgth.new[j]) # Transform coeff's back
  }
  V.coeff
}
...
lambda.grid = seq(10/sqrt(n.obs), 30/sqrt(n.obs), 1/sqrt(n.obs))   # Define grid of lambdas
  ...
  for (l in 1:length(lambda.grid)){
    loglik[l] = (n.obs/2 * log(2 * pi * 1) - (1/(2 * 1) 
                * t(Y[[s]] - design.tmp[[l]] %*% beta.tmp[[l]])
                ...
                - (n.obs * pen.prime(beta.prev[[l]] * weights.tmp[[l]], lambda.grid[l], a)
